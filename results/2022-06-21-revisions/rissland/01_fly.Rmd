...
bartel_zyg <- expr_genes <- read_excel(file.path(project_dir, 
                                                "docs",
                                                "bartel_2019_s1.xlsx"), 
                                       skip = 2) %>% 
...
for (i in seq_along(public_gene_sets)){
  ...
  splice_windows <- map(splice_regions, 
                      ...
                      all_res[[i]] <- foreach(i = bw_info,
                                     .packages = c("valr", "purrr", "dplyr", "rtracklayer"),
...
imap(all_plts,
     function(plts, study){
       ...
       save_plot(file.path(pfig_dir$sup_fig_1, 
                                str_c("intron_metagenes_from_", study,"_", y, ".pdf")), 
                      x, base_width = 6.5)
         })
       })
all_plts
