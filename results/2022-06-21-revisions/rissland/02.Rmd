...
gene2intron <- read_tsv(file.path(project_dir, "dbases", "rissland/eisa/tx2gene.tsv")) 
```
...
stage_cols <- stage_col_palette[1 + (levels(tpm_means$stage) >= "2-3 hr embryo")]
...
save_plot(file.path(pfig_dir$fig_1, "1d_fly-pre-mRNA-ratios.pdf"),
                    p, 
          base_asp = 1, base_height = 4.5, ncol = 2, nrow = 1)
...
save_plot(file.path(pfig_dir$fig_2, "2a_exon_intron_ma_plot.pdf"), 
          ma_plot, 
...
save_plot(file.path(pfig_dir$sup_fig_3, "sfig_3_maternal_cut_off_examination.pdf"),
          p,
          nrow = 5,
          ncol = 2,
          base_asp = 2,
...
rs_exon_intron_comps_log <- map(rs_exon_intron_comps_log, 
                                ~{colnames(.x) <- str_replace(colnames(.x), "embryo", "AEL")
                                  .x
                                })
...
h1 <- Heatmap(rs_exon_intron_comps_log$exon, 
                            na_col = "black",
              name = "Exon log2(TPM + 1)",
        show_row_dend = F, 
        show_column_dend = F,
        cluster_columns = FALSE,
        show_row_names = FALSE,
        column_title = "Exon TPM",
        split = rissland_mat_annot$mzt_class)

h2 <- Heatmap(rs_exon_intron_comps_log$intron, name = "Intron log2(TPM + 1)",
              na_col = "black",
        show_row_dend = F, 
        show_column_dend = F,
        cluster_columns = FALSE,
        show_row_names = FALSE,
        column_title = "Intron TPM",
        split = rissland_mat_annot$mzt_class)
...
pdf(file.path(pfig_dir$fig_2, "2e_eisa_all_classes.pdf"))
draw(h1 + h2)
dev.off()
...
mean_tpms_annot <- mean_tpms_annot %>% 
  ...
  mutate(mzt_class = str_c(mzt_class, 
                           ...
                           scales::comma(length(unique(gene_id))),
                           ")"),
         stage_ael = str_remove(stage, " hr embryo"))
p <- ggplot(mean_tpms_annot, aes(stage_ael, log2TPM)) +
    geom_boxplot(aes(fill = count_type),
                 notch = T, coef = NULL) +
    scale_fill_brewer(palette = "Set1") + 
    scale_fill_manual(values = grey_red) + 
    facet_grid(~mzt_class) +
    labs(x = "Hours after egg laying",
         y = "log2(TPM)",
         fill = "") +
...
save_plot(file.path(pfig_dir$fig_2, "2b_tpm_distribution_mztclasses.pdf"),
          p,
          base_asp = 2.3,
          limitsize = FALSE)
...
```{r}
mean_tpms_annot %>%
  group_by(stage_ael, count_type, mzt_class) %>% 
  summarize(mean_tpm = signif(mean(TPM), 5))
```
...
rs_exon_intron_comps_symbol <- lapply(rs_exon_intron_comps_symbol, 
                                      function(x){
                                        colnames(x) <- str_remove(colnames(x), " hr embryo")
                                        x 
                                      })
to_plot <- c(
  "clt",
...
mat_zyg_cols <- c("#F7941D", "#00A875", "#0072BC") 
plts <- map2(to_plot, rep(mat_zyg_cols, each = 2), 
    ~plot_exon_intron_expr(.x, 
                           rs_exon_intron_comps_symbol, 
                           line_width = 1.5,
                           line_col = .y) +
      labs(x = "Hours after\negg laying") + 
      theme(strip.background = element_blank()))
...
save_plot(file.path(fig_dir, "exon_intron_example_genes.pdf"),
          plt,
          nrow = 1, ncol = 6, base_asp = 0.85,
          base_height = 4)

save_plot(file.path(pfig_dir$fig_2, "2d_exon_intron_example_genes.pdf"),
          plt,
          nrow = 1, ncol = 6, base_asp = 0.6,
          base_height = 4.5)
```
...
# remove 0 intron counts from genes without introns
dat <- filter(dat, !is.na(dat$length))
...
p1 <- bind_rows(list("F_count" = f_classfieid,
  ...
  scale_fill_manual(values = grey_blue) + 
...
blue_cols <- c("#C7D6EE", "#659AD2", "#2484C6")
p2 <- plt_explain %>% 
  mutate(
  ...
  scale_fill_manual(values = rev(blue_cols)) + 
  labs(x = "",
...
plot_grid(p2, p1, p3, ncol = 3, rel_widths = c(0.75, 1, 1)) %>% 
  save_plot(file.path(pfig_dir$sup_fig_4, "s4abc_zyg_genes_missed_by_our_pipeline.pdf"),
            .,
            nrow = 1,
            ncol = 3,
            base_asp = 1.2)
...
ps <- map(to_plot,
       ~{set.seed(42)
         ggplot(sample_n(plt_dat, size = nrow(plt_dat)),
                aes_string(str_c("`", .x, "_traditional_method", "`"),
                    str_c("`", .x, "_our_method", "`"))) +
  geom_point(aes(fill = count_type), shape = 21, stroke = 0.4, size = 1, colour = "white") +
  scale_fill_manual(values = grey_red, name = "") + 
  geom_abline(slope = 1, linetype = 'dashed') +
  scale_x_log10(labels = scales::comma) + 
  scale_y_log10(labels = scales::comma) +
...
save_plot(file.path(pfig_dir$sup_fig_4, "s4d_comp_tpms_traditional_v_our_pipeline.pdf"),
          p,
          base_asp = 1.2,
          nrow = 1,
          ncol = 5)
...
early_zyg <- read_delim(file.path(project_dir, 
                "docs", "derenzis_sup_tbl_8.txt"),
                col_names = F, skip = 1, delim = " ")
early_zyg <- dplyr::select(early_zyg, gene_id = X2) %>% 
  na.omit() %>% 
  filter(str_detect(gene_id, "^FBgn")) %>% 
  left_join(gene2symbol, by = "gene_id") 

all_zyg <- read_delim(file.path(project_dir, 
                "docs", "derenzis_sup_tbl_1.txt"),
                col_names = F, skip = 2, delim = " ")
all_zyg <- dplyr::select(all_zyg, gene_id = X2) %>% 
  na.omit() %>% 
  filter(str_detect(gene_id, "^FBgn")) %>% 
  left_join(gene2symbol, by = "gene_id")

mat_zyg <- read_delim(file.path(project_dir, 
                "docs", "derenzis_sup_tbl_6.txt"),
                col_names = F, skip = 2, delim = " ")
...
id_update <- read_tsv(file.path(project_dir, 
                "docs", "Updated_FlyBase_IDs.txt"), 
...
red_blue_alpha <- c("#ED1C2460", "#0072BC60")
p <- plot(a, 
          fills = red_blue_alpha, legend = TRUE,
...
pdf(file.path(pfig_dir$fig_3, "3a_fcount_v_intron_count_classification_overlap.pdf"))
p
dev.off()
...
set.seed(4)
a <- euler(final_comparision, shape = 'ellipse')
...
list(
  ...
  `Kwasnieski et al` = b_genes$gene_id,
   `Lott et al` =  union(edata_genes$zyg$gene_id, 
                                edata_genes$matzyg$gene_id)) %>%
  saveRDS("processed_files/published_zyg_gene_lists.rds")

list(`Intron method` = gene_class_df,
     `Fcount Intron method` = fcount_gene_class_df ) %>% 
  saveRDS("processed_files/traditional_count_v_intron_count_lists.rds")
...
to_comp_upset <- list(
  ...
  `Lott, et al.` =  union(edata_genes$zyg$gene_id, 
                                edata_genes$matzyg$gene_id),
  `Intron read counting` = fcount_gene_class_df %>% filter(mzt_class == "all_zygotic_combined") %>% pull(gene_id)
) %>% 
  map(~.x[.x %in% gtf$gene_id] %>%  na.omit(.) %>% .[!duplicated(.)])

m = make_comb_mat(to_comp_upset)
p <- UpSet(m, 
      set_order = c("Intron read counting", 
                    "De Renzis, et al.", 
                    "Kwasnieski, et al.", 
                    "Lott, et al."),
...
pdf(file.path(fig_dir, "fcount_method_classification_overlap_upset.pdf"),
    width = 4.5, 
    height = 3)
p
dev.off()

pdf(file.path(pfig_dir$sup_fig_5, "fcount_method_classification_overlap_upset.pdf"),
    width = 4.5, 
    height = 3)
p
dev.off()
...
to_comp_upset <- list(
  ...
  `Lott, et al.` =  union(edata_genes$zyg$gene_id, 
                                edata_genes$matzyg$gene_id),
  `This paper` = gene_class_df %>% filter(mzt_class == "all_zygotic_combined") %>% pull(gene_id)
) %>% 
  map(~.x[.x %in% gtf$gene_id] %>%  na.omit(.) %>% .[!duplicated(.)])

m = make_comb_mat(to_comp_upset)
p <- UpSet(m, 
      set_order = c("This paper", 
                    "De Renzis, et al.", 
                    "Kwasnieski, et al.", 
                    "Lott, et al."),
      top_annotation = upset_top_annotation(m, add_numbers = TRUE,
                                            numbers_rot =90),
    right_annotation = upset_right_annotation(m, add_numbers = TRUE))
pdf(file.path(fig_dir, "intron_method_classification_overlap_upset.pdf"),
    width = 4.5, 
    height = 3)
p
dev.off()

pdf(file.path(pfig_dir$fig_3, "3b_intron_method_classification_overlap_upset.pdf"),
    width = 4.5, 
    height = 3)
p
dev.off()
...
```{r}
all_vals <- unique(unlist(final_comparision))

intersection_summary <- imap_dfc(final_comparision, 
...
intersection_summary <- mutate(intersection_summary, 
       discovered_class = case_when(
         `Intron method` & !`De Renzis et al` & !`Kwasnieski et al` & !`Lott et al` ~ "This paper",
         !`Intron method` & `De Renzis et al` & !`Kwasnieski et al` & !`Lott et al` ~ "De Renzis, et al.",
         !`Intron method` & !`De Renzis et al`& `Kwasnieski et al` & !`Lott et al` ~ "Kwasnieski, et al.",
         !`Intron method` & !`De Renzis et al`& !`Kwasnieski et al` &  `Lott et al` ~ "Lott, et al.",
         TRUE ~ "other"
       )) %>% 
...
intersection_summary <- mutate(intersection_summary, 
                               discovered_class = factor(discovered_class,
                                                         levels = c("This paper", "De Renzis, et al.", "Kwasnieski, et al.", "Lott, et al.")))
...
names(study_cols) <- levels(intersection_summary$discovered_class)

expr_plt_dat <- filter(intersection_summary, !is.na(discovered_class))

stat.test <- expr_plt_dat %>%
  wilcox_test(baseMean ~ discovered_class) %>% 
  add_significance() %>% 
  add_xy_position(y.trans = log10)

p <- ggplot(expr_plt_dat, aes(discovered_class, baseMean)) +
...
save_plot(file.path(pfig_dir$fig_3, "3c_expression_comparision_between_groups.pdf"),
          p,
          base_asp = 1.2,
          base_height = 4)
p
...
All comparisions are significant. 
...
save_plot(file.path(pfig_dir$fig_3, "3d_intron_count_comparision_between_groups.pdf"),
          p,
          base_asp = 1.5)
p
...
len_plt_dat <- filter(intersection_summary, discovered_class != "other")

stat.test <- len_plt_dat %>%
  wilcox_test(primary_tx_length ~ discovered_class) %>% 
  add_significance() %>% 
  add_xy_position(y.trans = log10)

p <-  ggplot(len_plt_dat, aes(discovered_class, primary_tx_length)) +
    scale_y_log10(labels = scales::comma) + 
    geom_violin(aes(fill = discovered_class)) +
    scale_fill_manual(values = study_cols) + 
  labs(x = "",
...
save_plot(file.path(pfig_dir$fig_3, "3e_primary_tx_length_comparision_between_groups.pdf"),
          p,
          base_asp = 1.2,
          base_height = 4)
p
stat.test
```
All comparisions are significant except DeRenzis, et al. v.	Kwasnieski, et al.
...
go_res <- intersection_summary %>% 
  filter(discovered_class == "This paper") %>% 
  pull(gene_id) %>% 
  gost(.,
...
```{r eisen_zelda_dat}
# use combined data
zelda_chip <- read_xls(file.path(project_dir, "docs", "eisen_zelda_chip_master_table.xlsx")) %>% 
...
all_plt_dat <- bind_rows(
  list(`Classification` = all_plt_dat, 
       ...
       .id = "type") %>% 
  mutate(mzt_class = str_remove(mzt_class, " hr embryo"))
...
p <- ggplot(all_plt_dat, aes(mzt_class, zelda_signal)) + 
  ...
  labs(x = "",
       y = "Zelda Chip-Seq Signal") +
  scale_fill_manual(values = c(blue_cols, blue_cols[3], mat_zyg_cols)) +
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 90,
                                   hjust = 1,
                                   vjust = 0.5),
...
save_plot(file.path(pfig_dir$fig_2, "2c_zelda-binding.pdf"),
          p,
          base_asp = 1,
          base_height = 5)
...
dplyr::select(stat.test, 
              ...
              n1,n2) %>% 
  write_csv(file.path(pfig_dir$fig_2, "zelda-binding-stats.csv"))
plt_txt
```
...
openxlsx::write.xlsx(list(README = col_info,
                          mzt_classifications = out_summary_table),
                     file.path(ptable_dir, "table_s2.xlsx"),
                     overwrite = TRUE)
