...
pdf(file.path(pfig_dir$fig_4, "4c_tx_isoform_mechanism_upset.pdf"),
    width = 4.5, 
    height = 3)
p
dev.off()
...
# table also has yeast proteins, only keep fly matched peptides 
# note that this didn't change the # of matching peptides
peps <- peps[startsWith(peps$Leading.razor.protein, "FBtr"), ]
...
res_out %>% 
  ...
  mutate(source = str_replace_all(source, '[[:punct:]]+', " ")) %>% 
  split(., .$source) %>% 
openxlsx::write.xlsx(., file.path(ptable_dir, "table_s4.xlsx"), overwrite = TRUE)
```
...
colnames(tx_tpms) <- str_replace(colnames(tx_tpms), "embryo", "AEL")
...
pdf(file.path(pfig_dir$fig_4, "4b_mat_v_zyg_isoforms_heatmap.pdf"), 
    height = 7, 
    width = 7)
  draw(ht_gene + ht_pre)
dev.off()
...
openxlsx::write.xlsx(list(README = README_txt,
                          ...
                          mat_zyg_isoforms_CDS_changes = cds_changed_annots,
                          all_isoforms = all_tx_data_out),
                     file.path(ptable_dir, "table_s3.xlsx"),
                     overwrite = TRUE)
...
mat_zyg_red_cols <- c(mat_zyg_cols, "#ED1C24")
mat_zyg_grey_cols <- c(mat_zyg_cols[2:3], "#808285")
```
...
mass_spec_evidence <- filter(isos,
  ...
  mutate(peptide_evidence = case_when(
    is.na(pep_tx) ~ "Non-unique CDS\nisoform",
    map2(featureID, 
         str_split(pep_tx, ","), 
         ~.x %in% .y) %>% 
      unlist() ~ "Mass spec peptides\nobserved",
    pep_tx == "no match" ~ "No mass spec\npeptides observed",
    TRUE ~ "No mass spec\npeptides observed"))

p <- ggplot(mass_spec_evidence, aes("1")) +
  geom_bar(aes(fill = peptide_evidence))  +
  scale_fill_manual(values = mat_zyg_grey_cols, name = "") +
  labs(x = "",
       y = "# of proteins",
       fill = "") + 
...
save_plot(file.path(pfig_dir$fig_5, "5c_mass_spec_overlap.pdf"),
          p,
          base_asp = 0.9)
```
...
pretty_rename <- c(
  "domain sequence changed" = "Domain\nsequence\nchanged",
  "same" = "Other CDS change",
  "loss" = "Domain loss",
  "gain" = "Domain gain",
  "gain_and_loss" = "Domain gain\nand loss")
...
p <- ggplot(plt_dat,
  ...
  scale_fill_manual(values = c(mat_zyg_red_cols, "#808285")[c(2, 4, 5, 3, 1)],
                    name = "") +
  labs(x = "",
       y = "# of proteins") + 
  theme(axis.text.x = element_blank())
...
save_plot(file.path(pfig_dir$fig_5, "5b_cds_domain_change_summary.pdf"), p,
          base_asp = 0.8)
```
...
pdf(file.path(pfig_dir$fig_5, "5a_utr_cds_overlap_isoforms_upset.pdf"),
    width = 4.5, 
    height = 3)
p
dev.off()
...
plt_dat <- ribo_summary %>% 
  ...
  mutate(evidence = case_when(
    is.na(total_rpfs) ~ "Non-unique CDS\nisoform",
    total_rpfs > 10 ~ ">= 10 RPFs observed\nin Ribo-seq",
    TRUE ~ "< 10 RPFs observed\nin Ribo-seq"),
         x = "1") 

p <- ggplot(plt_dat,
       aes(x)) +
  geom_bar(aes(fill = evidence)) +
  scale_fill_manual(values = mat_zyg_grey_cols[c(3,1,2)], name = "") + 
  labs(x = "",
       y = "Number of coding regions") +
  theme(axis.text.x = element_blank())
...
save_plot(file.path(pfig_dir$fig_5, "5d_riboprofiling_coverage.pdf"),
          p,
          base_asp = 1.0)
```
