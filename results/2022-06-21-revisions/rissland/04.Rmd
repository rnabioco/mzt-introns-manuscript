...
found_early_genes <- unique(intersect(our_zyg, derenzis_kwas_genes))
length(found_early_genes)

gtf <- import(annots$drosophila$gtf)
gtf[gtf$gene_name %in% found_early_genes] %>% 
  as.data.frame() %>% 
  filter(type == "exon") %>% 
  ...
  summarize(n_exons = n(),
            n_introns = n_exons - 1) %>% 
  group_by(gene_name) %>% 
  summarize(max_n_introns = max(n_introns)) %>% 
  group_by(max_n_introns) %>% 
  summarize(n())
```

We identify 28 of the 50 early zygotic genes identified in both the De Renzis and Kwasnieski papers. 
...
p <- fcount_intron_gl %>% 
  ggplot(aes(gene_class, exon_baseMean)) +
    ...
    scale_y_log10(labels = scales::comma) + 
    scale_fill_manual(values = study_cols) + 
   #   stat_pvalue_manual(stat.test, label = "{p.adj.signif}", size = 2, step.increase = 0.01) +
  labs(x = "",
       y = "Normalized exon\nexpression") +
    theme(legend.position = "none",
        axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
...
Examine go-terms for gene sets

```{r, eval = FALSE}
library(gprofiler2)

gl <- list("This Paper" = setdiff(zyg_gene_lists$`Intron method`, 
                                      zyg_gene_lists$`Fcount Intron method`),
          "Traditional\nmapping" = setdiff(zyg_gene_lists$`Fcount Intron method`,
                                      zyg_gene_lists$`Intron method`),
          "Shared genes" = intersect(zyg_gene_lists$`Intron method`,
...
gos <- lapply(gl, get_go_terms)
sig_terms <- lapply(gos, function(x) x[[2]] %>% filter(source == "GO:BP", p_value < 0.05) %>% slice(1:200) %>%  pull(term_name)) %>% unlist() %>% unique()

go_df <- lapply(gos, function(x) {
  gt <- x[[2]]
  gt <- filter(gt, source == "GO:BP", term_name %in% sig_terms) 
  gt <- data.frame(neg_log10_P = gt$negative_log10_of_adjusted_p_value, row.names = gt$term_name)
  gt[sig_terms, , drop = FALSE]
}) %>% 
  do.call(cbind, .)
