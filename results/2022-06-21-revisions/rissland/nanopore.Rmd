...
read_length_stats %>% 
  ...
  split(., .$sample) %>% 
  map(~pull(.x, qname)) %>% 
  iwalk(~writeLines(.x, file.path(out_dir, str_c(.y, "full_length_reads.txt"))))
...
p <- mutate(switch_isos_summary, 
  ...
  ggplot(aes(transcript_type, `# of transcripts`)) + 
    ...
    scale_fill_manual(values = c("#00A875", "#808285"), name = "") +
  labs(x = "")
...
save_plot(file.path(pfig_dir$fig_4, "4e_nanopore_support.pdf"),
          p,
          base_asp = 1.2)
```
