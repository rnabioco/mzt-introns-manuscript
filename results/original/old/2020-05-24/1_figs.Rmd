...
gene2intron <- read_tsv(file.path(eisa_dir, "tx2gene.tsv"))
```
...
```{r eval = FALSE}
rs_tx_objs[[1]]$abundance  %>% 
#fpm(rs_ddobjs$eisa_masked_bt2, robust = FALSE) %>% 
  as.data.frame() %>% 
  set_names(str_c(pdata_split$RISSLAND$developmental_stage, "_", pdata_split$RISSLAND$strain)) %>% 
  rownames_to_column("gene") %>% 
  pivot_longer(cols = -gene, names_to = "expt", values_to = "tpm") %>% 
  separate(expt, c("stage", "strain"), sep = "_") %>% 
 mutate(
    lib_type = ifelse(
      strain == "wt",
      "Poly(A)+",
    ...
    count_type = ifelse(
      str_detect(gene, "-I[0-9]*$"),
      "premRNA",
      "mRNA"
    ),
    gene = str_remove(gene, "-I[0-9]*$"),
  ) %>% 
  pivot_wider(names_from = count_type, values_from = tpm) %>% 
  mutate(primary_ratio = premRNA / (mRNA + premRNA)) %>% 
  na.omit() %>% 
  group_by(gene, stage, lib_type) %>% 
  summarize(mean_primary = mean(primary_ratio)) %>% 
  ggplot(aes(stage, mean_primary)) +
  geom_boxplot(aes(fill = stage), outlier.shape = NA, coef = 1e10) + 
  facet_grid(~lib_type, scale = "free") +
  labs(x = "",
       y = expression(frac("pre-mRNA TPM", 
...
tmp_plt_dat <- bind_rows(mean_intronic_tpms, mean_exonic_tpms) %>% 
  ...
  arrange(count_type) %>%
  filter(padj < 0.05) %>% 
  mutate(log2FoldChange = ifelse(log2FoldChange >  plot_fc_cut_off,
                                 plot_fc_cut_off,
                                 ifelse(log2FoldChange < -plot_fc_cut_off,
                                        -plot_fc_cut_off,
                                        log2FoldChange)))
...
save_plot(file.path(fig_dir, "avergage_expr_time_purezygotic_staged_zscore.pdf"),
          pavg_zscore,
          base_height = 7.5,
          base_asp = 0.66,
          nrow = 1,
